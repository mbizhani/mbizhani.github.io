...
val b = ArrayBuffer[Int]() // Or new ArrayBuffer[Int], An empty array buffer
...
println(fromJavaArr.mkString("{", ", ", ")"))      // {Hello, World)
println(myJava.sumArrayOfInteger(Array(1, 2, 4)))  // 7
...
println(s"idx = ${binarySearch(arr.asInstanceOf[Array[Object]], "b")}") // idx = 1
println(s"idx = ${arr.search("c")}")  // idx = InsertionPoint(2)
...
```
- Line 15, call Java's `binarySearch` and passing an array of String, however a type conversion is necessary
- Line 16
...
- Line 24-26: two steps of conversion from Java to Scala
  - Line 25: call `asScala` (applicable on all Java collections) to convert to Scala equivalent
  - Line 26: call `map` to convert Java's `Integer` to Scala's `Int`