...
- A lambda can may have no parameter (e.g. `Runnable`)
...
public class Test {
  public static void main(String[] args) {
    List<String> strings = Arrays.asList("car", "A", "Bill", "Bar");
    ...
    // or
    Collections.sort(strings, Test::staticComp); // Class::staticMethod
    // or
    MyCompClass inst = new MyCompClass(strings);
    Collections.sort(strings, inst::comp); // object::instanceMethod
    // or
  ...
  static int staticComp(String a, String b) {
    return Integer.compare(a.length(), b.length());
  }

  static void repeatMessage(String text, int count) { // 
    Runnable r = () -> {
      for (int i = 0; i < count; i++) { // free value of 'count', can't call count-- 
        System.out.println(text);       // free value of 'text'
        Thread.yield();
      }
    };
    new Thread(r).start();
  ...
  static class MyCompClass implements Runnable {
    private List<String> list;

    MyCompClass(List<String> list) {
      this.list = list;
    }
    ...
    @Override
    public void run() {
      list.sort(this::comp); // this::instanceMethod
    }
  }
}
```
...
public class Test {
  public static void main(String[] args) throws Exception {
    Thread myThread = new Thread(Test::go, "MyThread");
    myThread.start();
    myThread.join();

    System.out.println("End of Main, thread name=" + 
      Thread.currentThread().getName()); // thread name=main
  }
  
  static void go() {
    System.out.println("go() , thread name=" + 
      Thread.currentThread().getName()); // thread name=MyThread
  }
}
```
...
#### Working with `Files`
```java
final Consumer<File> printFileName = f -> System.out.println(f.isDirectory() ? f.getName() + "/" : f.getName());
