...
DOMAIN_BASE=mydomain.local
GITLAB_DOMAIN="gitlab.${DOMAIN_BASE}"
NEXUS_DOMAIN="nexus.${DOMAIN_BASE}"

GITLAB_VER="13.6.0-ce.0"
GITLAB_RUNNER_VER="v13.6.0"
NEXUS_VER="3.28.1"
TRAEFIK_VER="v2.3.3"
```
...
services:
  ...
  gitlab:
    ...
    labels:
      ...
      - "traefik.http.routers.gitlab.rule=Host(`${GITLAB_DOMAIN:?err}`)"
    volumes:
  ...
  nexus:
    ...
    labels:
      ...
      - "traefik.http.routers.nexus.rule=Host(`${NEXUS_DOMAIN:?err}`)"
      ...
      - "traefik.http.routers.nexusDocker.rule=Host(`${NEXUS_DOMAIN:?err}`) && PathPrefix(`/v2/`)"
    ports:
      # Docker Group Repo
      - 8082:8082
      # Docker Private Repo
